 ...
 */
static const char* BITCOIN_PID_FILENAME = "nullcoind.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/nullcoin/nullcoin>";
...
/** Sanity checks
 *  Ensure that Nullcoin is running in a usable environment with all
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if nullcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if nullcoin is started while in a temporary directory.\n",
