...
const char * const BITCOIN_CONF_FILENAME = "nullcoin.conf";
 ...
 * https://github.com/nullcoin/nullcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //nullcoin-tx using stdin
        std::string val;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Nullcoin";
#else
    // Unix
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Nullcoin Core copyright is not removed by accident
    if (copyright_devs.find("Nullcoin Core") == std::string::npos) {
